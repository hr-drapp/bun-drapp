...
1. Create/update DB model in `src/models/...` (or `src/models/drapp/...`).
...
Drapp:
- `drapp/Clinic`, `drapp/Doctor`, `drapp/Patient`, `drapp/AutoIncementalId`
...
- Auto increment ids are managed by `drapp/AutoIncementalId` + `GetAutoIncrId(...)`.
